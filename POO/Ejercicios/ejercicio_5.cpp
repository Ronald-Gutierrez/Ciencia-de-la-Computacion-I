...
Lavadora::~Lavadora(){}
Lavadora::Lavadora(float _precio_base, string _color, string _consumo_energetico, float _peso, int _carga):Electrodomestico(_precio_base,_color,_consumo_energetico,_peso){
    carga=_carga;
}
void Lavadora::setCarga(int _carga){
    carga=_carga;
}
int Lavadora::getCarga(){
    return carga;
}
void Lavadora::setPrecioFinal2(int _precio_final2){
    precio_final2=_precio_final2;
}   
int Lavadora::getPrecioFinal2(){
    return precio_final2+getCarga()*10+getPrecioFinal();
}
void Lavadora::mostrarLavadora(){
    mostrar();
    cout<<"La carga es de: "<<getCarga()<<endl;
    cout<<"El precio final de la lavadora es: "<<getPrecioFinal2()<<endl;
}
